...
{
	public partial class ProductDetailForm2 : Form
	{
		...
		bool isAdd;
		public ProductDetailForm2(int productId, int memberId, int count)
		{
			InitializeComponent();
			...
			_count = count;
			originCount = _count;
			this.Load += ProductDetailsForm2_Load;
		...
		private void BindData()
		{
			...
			int maxLength = 19;
			if (length.Length > maxLength) //當產品名長度大於19自動換行
			{
		...
		public OrderDto GetUnconfirmedOrder(int memberId) // 檢查訂購單處理狀態
		{
		...
		{
			GetOrderItemService().Delete(_productId);
